...
id_try_combination = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(list))))
...
y = []
for id_try in id_try_combination:
    matrix = [[0 for i in range(biggest - least + 1)] for j in range(biggest - least + 1)]
    user_name_temp = ''
    for user_name in id_try_combination[id_try]:
...
with torch.no_grad():
    ...
    print(f'Test Accuracy of the model on the {len(X)} test records: {(correct / total) * 100} %')
